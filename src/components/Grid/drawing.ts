    ...
    const offset = (d) => {
      ...
      moveable = d3
        ...
        .on("start", function (event) {
          event.sourceEvent.stopPropagation(); // don't zoom when I'm draggin' an image!
          [startX, startY] = d3.pointer(event, document.querySelector("#grid svg"));
          currX = currY = undefined;
        })
        .on("drag", function (event, d) {
          [currX, currY] = d3.pointer(event, document.querySelector("#grid svg"));
