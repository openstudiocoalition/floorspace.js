...
import {
  ...
  snapWindowToEdge,
  windowLocation,
} from '../../../../src/components/Grid/snapping';
import {
  assertProperty,
  assertEqual,
  assert,
...
describe('gridSnapTargets', () => {
  it('returns 4 unique points', () => {
    assertProperty(genPoint, gen.intWithin(1, 100), (pt, gridSpacing) => {
...
describe('snapTargets', () => {
  ...
  it('prioritizes a vertex over a grid point that has same distance', () => {
    const targets = snapTargets([{ x: 0.4, y: 0.4 }], 1, { x: 0.4, y: 0 }),
      first = targets[0],
      second = targets[1];
...
describe('snapWindowToEdge', () => {
  ...
  it('returns null on empty window list', () => {
    refute(
      snapWindowToEdge(
        ...
        [],
        { x: 12, y: 12 },
        { width: 10, window_definition_mode: 'Single Window' },
        inclusiveMaxSnapDist,
      ),
    );
  });
  ...
  it('returns a segment centered on an edge', () => {
    assertProperty(genPointInNeighborhood, (cursor) => {
  ...
  it('alpha accurately describes the location of the center', () => {
    assertProperty(genPointInNeighborhood, (cursor) => {
      ...
      assert(
        isNearlyEqual(alphaAlongEdge, _.pick(windowLoc.center, ['x', 'y'])),
      );
    });
  });
  ...
  it('returns a segment with the same angle as an edge', () => {
    assertProperty(genPointInNeighborhood, (cursor) => {
      ...
      assert(
        ...
        `expected ${JSON.stringify(edge)} and ${JSON.stringify(
          windowLoc,
        )} to be parallel`,
      );
    });
    ...
    assertProperty(
      ...
      (cursor, windowWidth) => {
        const windowLoc = snapWindowToEdge(
          ...
          edges,
          cursor,
          { width: windowWidth, window_definition_mode: 'Single Window' },
          inclusiveMaxSnapDist,
        );
        assert(
          nearlyEqual(
