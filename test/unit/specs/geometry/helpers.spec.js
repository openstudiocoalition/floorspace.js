...
import helpers, {
  fitToAspectRatio,
  ringEquals,
} from '../../../../src/store/modules/geometry/helpers';
import {
  assert,
  refute,
  nearlyEqual,
...
describe('ringEquals', () => {
  it('identifies truly equal vertex lists', () => {
    assertProperty(genIrregularPolygon, (pts) => ringEquals(pts, pts));
  });

  it('ignores id property', () => {
    assertProperty(
      genIrregularPolygon.then((pts) => ({
  ...
  it('is independent of rotation', () => {
    assertProperty(genIrregularPolygon, gen.posInt, (pts, pivot) => {
  ...
  it('is independent of winding order', () => {
    assertProperty(genIrregularPolygon, (pts) => {
    ...
    assertProperty(
      gen
        ...
        .then(({ pts, pivot }) => {
          const pivotIx = pivot % pts.length;
          const ptsReversed = [...pts].reverse();
          const ptsReversedAndRotated = [
            ...ptsReversed.slice(pivotIx),
            ...ptsReversed.slice(0, pivotIx),
          ];
          return {
            a: [...pts, pts[0]],
  ...
  it('makes no changes to parameters', () => {
    assertProperty(genIrregularPolygon, genIrregularPolygon, (a, b) => {
      const origA = [...a],
        origB = [...b];
      ringEquals(a, b);
...
describe('projectionOfPointToLine', () => {
  const lineUR = { p1: { x: -5, y: -5 }, p2: { x: 5, y: 5 } },
  ...
  it('projects points collinear with the line to an endpoint', () => {
    const collinearPtsAfterLine = gen
        .numberWithin(5, 10000)
        .then((num) => ({ x: num, y: num })),
      collinearPtsBeforeLine = gen
        .numberWithin(-10000, -5)
        .then((num) => ({ x: num, y: num }));

    assertProperty(collinearPtsAfterLine, (pt) => {
      const proj = helpers.projectionOfPointToLine(pt, lineUR);
      return nearlyEqual(proj.x, 5) && nearlyEqual(proj.y, 5);
    });

    assertProperty(collinearPtsBeforeLine, (pt) => {
      const proj = helpers.projectionOfPointToLine(pt, lineUR);
      return nearlyEqual(proj.x, -5) && nearlyEqual(proj.y, -5);
    });
  });
...
describe('setOperation', () => {
  it('union or intersection with self is self', () => {
    assertProperty(genTriangle, (tri) => {
  ...
  it('does not allow holes', () => {
    assertProperty(
      ...
      ({ numEdges, radius, center }, scalingFactor) => {
        ...
        const difference = helpers.setOperation(
          'difference',
          polygon,
          innerPolygon,
        );
    ...
    const points = [
      {
        ...
        y: 305,
        type: 'grid',
        dist: 2.511988458940511,
        dx: -2.435064935059998,
        dy: -0.6168831168799898,
      },
      { x: -55, y: 305 },
      {
        x: -55,
        y: 220,
        type: 'grid',
        dist: 2.7476818300832977,
        dx: 2.240259740260001,
        dy: 1.590909090910003,
      },
      { x: -125, y: 220 },
    ];
    const currentGeom = {
...
describe('splittingVerticesForEdgeId', () => {
  it('respects the spacing option for small geometry', () => {
    ...
    f +---------+ g
*/
    const edge = geometryExamples.smallGeometry.edges.find(
      (e) => e.id === 'ce',
    );
    const splittingVertices = helpers.splittingVerticesForEdgeId(
      edge,
      geometryExamples.smallGeometry,
      0.1,
    );
    assert(
      splittingVertices.length === 1,
      `should find one splitting vertex, but found ${splittingVertices.length}`,
    );
    assertEqual(splittingVertices[0], { id: 'd', x: 0.4, y: 1 });

    const splittingSmallerVertices = helpers.splittingVerticesForEdgeId(
      edge,
      geometryExamples.evenSmallerGeometry,
      0.01,
    );
    assert(
      splittingSmallerVertices.length === 1,
      `should find one splitting vertex, but found ${splittingSmallerVertices.length}`,
    );
...
describe('edgeDirection', () => {
  ...
  it('can handle a straight up or straight down line', () => {
    const north = helpers.edgeDirection({
        start: { x: 0, y: 0 },
        end: { x: 0, y: 1 },
      }),
  ...
  it('can handle a west or east line', () => {
    const east = helpers.edgeDirection({
        start: { x: 0, y: 0 },
        end: { x: 1, y: 0 },
      }),
    ...
    assert(nearlyEqual(east, 0), 'expected angle of east to be 0 rad');
    assert(
      nearlyEqual(west, 0),
...
describe('denormalize', () => {
  it('is undone by normalize', () => {
    assertProperty(
      ...
      (geom) => {
        assertEqual(geom, helpers.normalize(helpers.denormalize(geom)));
      },
  ...
  it('adding a face persists after normalization', () => {
    assertProperty(
      ...
      (geom) => {
        ...
        const renorm = helpers.normalize(denorm),
        ...
        assertEqual(_.map(faceInQuestion.edgeRefs, 'edge_id'), [
          'new_edge1',
          'new_edge2',
          'new_edge3',
        ]);
