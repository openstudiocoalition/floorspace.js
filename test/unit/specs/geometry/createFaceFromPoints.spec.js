...
import {
  preserveRectangularityGeometry,
  simpleGeometry,
  smallGeometry,
  evenSmallerGeometry,
  emptyGeometry,
...
describe('validateFaceGeometry', () => {
  it('preserves rectangularity when possible (issue #72 )', () => {
    ...
    const points = JSON.parse(
      '[{"x":0,"y":1},{"x":3.10847804166,"y":1},{"x":3.10847804166,"y":2.75847898924},{"x":0,"y":2.75847898924}]',
    );

    const res = validateFaceGeometry(points, preserveRectangularityGeometry),
      newVerts = _.reject(res.vertices, (v) =>
        preserveRectangularityGeometry.vertices.find(
          (c) => helpers.distanceBetweenPoints(v, c) < 0.0001,
        ),
      );
  ...
  it('fails when given too few vertices', () => {
    assertProperty(genTooFewVerts, (verts) => {
  ...
  it('succeeds from empty on an irregular polygon', () => {
    assertProperty(genIrregularPolygon, (poly) => {
      const resp = validateFaceGeometry(poly, emptyGeometry);
      assert(resp && resp.success);
    });
  });

  const genZeroAreaTriangle = gen
      .array(genPoint, { size: 2 })
      .then(([a, b]) => [a, b, a]),
    genZeroAreaRectangle = gen
      .array(genPoint, { size: 2 })
      .then(([a, b]) => [a, b, b, a]),
    genZeroAreaPolygon = gen.oneOf([genZeroAreaTriangle, genZeroAreaRectangle]);

  it('fails when given a zero-area polygon', () => {
    assertProperty(genZeroAreaPolygon, (poly) => {
      const resp = validateFaceGeometry(poly, emptyGeometry);
      refute(resp && resp.success);
    });
  });

  const genPolygonWithSpur = gen
    .object({
      center: genPoint,
    ...
    .then(({ center, radii, spurPos: spurPosBeforeMod }) => {
      const spurPos = spurPosBeforeMod % radii.length,
...
describe('edgesToSplit:simpleGeometry', () => {
  ...
  */
  const edges = edgesToSplit(simpleGeometry, 5),
  ...
  edges.forEach(
    (payload) =>
      splitEdge(state, { geometry_id: simpleGeometry.id, ...payload }),
    5,
  );

  const oldGeom = helpers.denormalize(simpleGeometry),
    newGeom = helpers.denormalize(newSimpleGeometry);

  const vertsIdsFromFace = (face) =>
    _.chain(face.edges)
      .flatMap((e) => (e.reverse ? [e.v2, e.v1] : [e.v1, e.v2]))
      .map('id')
      .value();
  ...
  it('splits an edge in a simple case', () => {
    ...
    assert(
      edgeToDelete === 'ce',
      `that edge should be "ce" but was ${edgeToDelete}`,
    );

    const replaceCE = _.find(replaceEdgeRefs, {
      face_id: 'bottom',
      edge_id: 'ce',
    });
    assert(replaceCE);
    ...
    edges.forEach((edge) => {
      edge.replaceEdgeRefs.forEach((rer) => {
        ...
        newEdgeRefs.forEach((ner) => {
          ...
          assertEqual(ner.reverse, originalEdgeRef.reverse);
        });
      });
    });
  });
  ...
  it('produces closed faces', () => {
    newGeom.faces.forEach((face) => {
      ...
      _.chunk(verts.slice(1, -1), 2).forEach(([v2OfPrev, v1OfNext]) =>
        assertEqual(v2OfPrev, v1OfNext),
      );
    });
  });
});
  ...
  it('should not return edges that are empty', () => {
    ...
    edges.forEach((edge) => {
      edge.newEdges.forEach((newEdge) => {
        const vert1 = emptyEdgesProblem.vertices.find(
          (vert) => vert.id === newEdge.v1,
        );
        const vert2 = emptyEdgesProblem.vertices.find(
          (vert) => vert.id === newEdge.v2,
        );
        const distance = helpers.distanceBetweenPoints(vert1, vert2);
        assert(
          distance > 0,
          `distance must be greater than 0, but was ${distance}`,
        );
      });
    });
  });
...
describe('edgesToSplit:smallGeometry', () => {
  ...
  */
  const edges = edgesToSplit(smallGeometry, 0.1),
  ...
  edges.forEach(
    (payload) =>
      splitEdge(state, { geometry_id: smallGeometry.id, ...payload }),
    0.2,
  );

  const oldGeom = helpers.denormalize(smallGeometry),
  ...
  it('splits an edge in a simple case', () => {
    assert(
      edges.length === 1,
      `only one edge should be split but ${edges.length} were`,
    );
...
describe('edgesToSplit:evenSmallerGeometry', () => {
  ...
  */
  const edges = edgesToSplit(evenSmallerGeometry, 0.01),
  ...
  edges.forEach(
    (payload) =>
      splitEdge(state, { geometry_id: evenSmallerGeometry.id, ...payload }),
    0.01,
  );

  const oldGeom = helpers.denormalize(evenSmallerGeometry),
...
describe('newGeometriesOfOverlappedFaces', () => {
  const geometry = helpers.normalize({
      vertices: [],
      edges: [],
      id: 1,
      faces: [
