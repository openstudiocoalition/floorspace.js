...
describe('libconfig', () => {
  describe('definitions', () => {
    ...
    it('should be able to initialize a building type', () => {
      const buildingTypeInstance = libconfig.building_types.init({
        name: 'test',
      });

      assert(
        !!buildingTypeInstance.id,
        'No column for id found on building type',
      );
      assertEqual(buildingTypeInstance.name, 'test');
      assert(
        !!buildingTypeInstance.color,
        'No column for color found on building type',
      );
    });
  });
  ...
  describe('spaces', () => {
    it('should have a column definition for building type', () => {
      const buildingType = libconfig.spaces.columns.find(
        (c) => c.displayName === 'Building Type',
      );
      assert(
        !!buildingType,
        'Could not find column definition for building type',
      );
      ...
      const retrieveValue = buildingType.get(state.stories[2].spaces[0], {
        models: state,
      });
