...
describe('model mutations', () => {
  it("destroyObject doesn't delete extra stuff (issue #131)", () => {
    ...
    mutations.destroyObject(goodState, {
      object: { color: '#000000', id: '23', name: 'Thermal Zone 2' },
    });

    // deleting an unrelated object shouldn't have caused a change in spaceTypes.
    assertEqual(spaceTypeIds, _.map(goodState.library.space_types, 'id'));
  ...
  it('should update all spaces within the same building unit when the building type changes', () => {
    mutations.updateSpaceWithData(goodState, {
      space: goodState.stories[0].spaces[0],
      building_type_id: 'new_building_type',
    });
    assertEqual(
      goodState.stories[0].spaces[0].building_type_id,
      'new_building_type',
    );
    assertEqual(
      goodState.stories[0].spaces[1].building_type_id,
      'new_building_type',
    );
    assertEqual(
      goodState.stories[1].spaces[0].building_type_id,
      'new_building_type',
    );
  ...
  it('should update a space with the new building type when changing building unit', () => {
    assertEqual(
      goodState.stories[2].spaces[0].building_type_id,
      'old_building_type',
    );
    mutations.updateSpaceWithData(goodState, {
      space: goodState.stories[2].spaces[0],
